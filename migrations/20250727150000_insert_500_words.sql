INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'abate',
    'to reduce in amount, degree, or intensity',
    '/əˈbeɪt/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'aberrant',
    'departing from an accepted standard',
    '/æˈberənt/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'aberration',
    'a departure from what is normal, usual, or expected',
    '/ˌæbəˈreɪʃən/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'abstain',
    'restrain oneself from indulging in or enjoying something',
    '/æbˈsteɪn/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'abstemious',
    'practicing restraint, especially with food and drink',
    '/æbˈstiːmiəs/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'acclaim',
    'enthusiastic and public praise',
    '/əˈkleɪm/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'acquiesce',
    'accept something reluctantly but without protest',
    '/ˌækwiˈes/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'acrimonious',
    'angry and bitter in speech or manner',
    '/ˌækrɪˈmoʊniəs/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'adamant',
    'refusing to be persuaded or to change ones mind',
    '/ˈædəmənt/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'adjacent',
    'next to and joined with',
    '/əˈdʒeɪsənt/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'admonish',
    'to warn or reprimand someone firmly',
    '/ədˈmɑːnɪʃ/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'advent',
    'the arrival of a notable person, thing, or event',
    '/ˈædvent/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'adversity',
    'difficulties; misfortune',
    '/ædˈvɜːrsəti/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'advocate',
    'a person who publicly supports or recommends a particular cause',
    '/ˈædvəkət/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'aesthetic',
    'concerned with beauty or the appreciation of beauty',
    '/ɛsˈθɛtɪk/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'affable',
    'friendly, good-natured, or easy to talk to',
    '/ˈæfəbəl/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'affinity',
    'a spontaneous or natural liking or sympathy for someone or something',
    '/əˈfɪnəti/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'affluent',
    'having a great deal of money; wealthy',
    '/ˈæfluənt/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'aggregate',
    'a whole formed by combining several elements',
    '/ˈæɡrɪɡət/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'alacrity',
    'brisk and cheerful readiness',
    '/əˈlækrɪti/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'alleviate',
    'make suffering, deficiency, or a problem less severe',
    '/əˈliːvieɪt/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'allure',
    'the quality of being powerfully and mysteriously attractive',
    '/əˈlʊr/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'altruistic',
    'showing unselfish concern for others',
    '/ˌæltruˈɪstɪk/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'ambiguous',
    'open to more than one interpretation',
    '/æmˈbɪɡjuəs/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'ambivalent',
    'having mixed feelings or contradictory ideas',
    '/æmˈbɪvələnt/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'ameliorate',
    'to make something bad or unsatisfactory better',
    '/əˈmiːljəreɪt/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'amiable',
    'having a friendly and pleasant manner',
    '/ˈeɪmiəbəl/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'amplify',
    'increase the volume or intensity of',
    '/ˈæmplɪfaɪ/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'anachronism',
    'something belonging to a different time period',
    '/əˈnækrəˌnɪzəm/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'analogous',
    'comparable in certain respects',
    '/əˈnæləɡəs/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'anecdotal',
    'based on personal accounts rather than facts',
    '/ˌænɪkˈdoʊtəl/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'animate',
    'bring to life or give energy to',
    '/ˈænəmeɪt/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'animosity',
    'strong hostility or dislike',
    '/ˌænɪˈmɑːsɪti/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'annihilate',
    'destroy utterly; obliterate',
    '/əˈnaɪəleɪt/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'antagonist',
    'a person who actively opposes or is hostile to someone',
    '/ænˈtæɡənɪst/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'anthology',
    'a published collection of poems or other pieces of writing',
    '/ænˈθɑːlədʒi/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'anticipate',
    'regard as probable; expect or predict',
    '/ænˈtɪsəpeɪt/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'antiquated',
    'old-fashioned or outdated',
    '/ˈæntɪkweɪtɪd/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'antithesis',
    'a person or thing that is the direct opposite',
    '/ænˈtɪθəsɪs/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'apathy',
    'lack of interest, enthusiasm, or concern',
    '/ˈæpəθi/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'appease',
    'pacify or placate someone by acceding to their demands',
    '/əˈpiːz/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'apprehensive',
    'anxious or fearful about the future',
    '/ˌæprɪˈhɛnsɪv/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'arbitrary',
    'based on random choice rather than reason',
    '/ˈɑːrbɪˌtrɛri/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'archaic',
    'very old or old-fashioned',
    '/ɑːrˈkeɪɪk/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'ardent',
    'enthusiastic or passionate',
    '/ˈɑːrdənt/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'arrogant',
    'having or revealing an exaggerated sense of ones importance',
    '/ˈærəɡənt/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'articulate',
    'having or showing the ability to speak fluently',
    '/ɑːrˈtɪkjələt/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'ascertain',
    'find something out for certain; make sure of',
    '/ˌæsərˈteɪn/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'ascetic',
    'characterized by severe self-discipline',
    '/əˈsɛtɪk/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'aspiration',
    'a hope or ambition of achieving something',
    '/ˌæspəˈreɪʃn/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'assiduous',
    'showing great care and perseverance',
    '/əˈsɪdʒuəs/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'astute',
    'having or showing shrewdness',
    '/əˈstuːt/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'atrocity',
    'an extremely wicked or cruel act',
    '/əˈtrɑːsəti/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'attribute',
    'regard something as being caused by',
    '/əˈtrɪbjuːt/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'audacious',
    'showing willingness to take bold risks',
    '/ɔːˈdeɪʃəs/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'austere',
    'severe or strict in manner or attitude',
    '/ɔːˈstɪr/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'authentic',
    'of undisputed origin; genuine',
    '/ɔːˈθentɪk/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'autonomous',
    'having the freedom to act independently',
    '/ɔːˈtɑːnəməs/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'avarice',
    'extreme greed for wealth or material gain',
    '/ˈævərɪs/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'aversion',
    'a strong dislike or disinclination',
    '/əˈvɜːrʒn/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'banal',
    'so lacking in originality as to be obvious',
    '/bəˈnæl/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'bankruptcy',
    'the state of being bankrupt',
    '/ˈbæŋkrʌptsi/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'belligerent',
    'hostile and aggressive',
    '/bəˈlɪdʒərənt/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'benevolence',
    'the quality of being well meaning; kindness',
    '/bəˈnevələns/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'benevolent',
    'well-meaning and kindly',
    '/bəˈnɛvələnt/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'benign',
    'gentle and harmless in effect or influence',
    '/bɪˈnaɪn/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'bereft',
    'deprived of or lacking something',
    '/bɪˈrɛft/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'bigotry',
    'obstinate or unreasonable attachment to a belief',
    '/ˈbɪɡətri/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'blasphemy',
    'the act or offense of speaking sacrilegiously',
    '/ˈblæsfəmi/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'blatant',
    'done openly and unashamedly',
    '/ˈbleɪtənt/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'blithe',
    'showing a casual and cheerful indifference',
    '/blaɪð/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'bombastic',
    'high-sounding but with little meaning',
    '/bɑːmˈbæstɪk/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'boycott',
    'withdraw from commercial or social relations as punishment',
    '/ˈbɔɪkɑːt/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'brevity',
    'concise and exact use of words in writing or speech',
    '/ˈbrevəti/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'brusque',
    'abrupt or offhand in speech or manner',
    '/brʌsk/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'bureaucracy',
    'a system of government by many bureaus and petty officials',
    '/bjʊˈrɑːkrəsi/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'camaraderie',
    'mutual trust and friendship among people',
    '/ˌkæməˈrɑːdəri/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'candid',
    'truthful and straightforward; frank',
    '/ˈkændɪd/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'capitulate',
    'cease to resist an opponent or demand',
    '/kəˈpɪtʃəˌleɪt/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'capricious',
    'given to sudden changes of mood or behavior',
    '/kəˈprɪʃəs/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'catalyst',
    'a person or thing that precipitates change',
    '/ˈkætəlɪst/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'catastrophe',
    'an event causing great and sudden damage or suffering',
    '/kəˈtæstrəfi/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'caustic',
    'sarcastic in a scathing and bitter way',
    '/ˈkɔːstɪk/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'censor',
    'to examine and suppress unacceptable parts',
    '/ˈsɛnsər/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'charlatan',
    'a person falsely claiming special knowledge',
    '/ˈʃɑːrlətn/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'chastise',
    'to rebuke or reprimand severely',
    '/tʃæˈstaɪz/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'circumspect',
    'wary and unwilling to take risks',
    '/ˈsɜːrkəmˌspɛkt/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'clandestine',
    'kept secret or done secretively',
    '/klænˈdɛstɪn/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  ('clemency', 'mercy; lenience', '/ˈkleməsi/');

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'coalesce',
    'come together to form one mass or whole',
    '/ˌkoʊəˈles/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'cogent',
    'clear, logical, and convincing',
    '/ˈkoʊdʒənt/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'coherent',
    'logical and consistent',
    '/koʊˈhɪrənt/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'compelling',
    'evoking interest, attention, or admiration powerfully',
    '/kəmˈpelɪŋ/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'compendium',
    'a collection of concise but detailed information',
    '/kəmˈpɛndiəm/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'complacent',
    'showing smug satisfaction with oneself',
    '/kəmˈpleɪsənt/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'comprehensive',
    'complete and including everything that is necessary',
    '/ˌkɑːmprɪˈhensɪv/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'conciliatory',
    'intended to placate or pacify',
    '/kənˈsɪliəˌtɔːri/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'condescending',
    'having an attitude of patronizing superiority',
    '/ˌkɑːndɪˈsɛndɪŋ/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'conducive',
    'making a certain situation or outcome likely or possible',
    '/kənˈduːsɪv/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'congenial',
    'agreeable or suitable to one s nature',
    '/kən\ˈdʒiːniəl/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'conniving',
    'conspiring to do something immoral or illegal',
    '/kəˈnaɪvɪŋ/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'connoisseur',
    'an expert judge in matters of taste',
    '/ˌkɑːnəˈsɜːr/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'conscientious',
    'wishing to do what is right',
    '/ˌkɑːnʃiˈɛnʃəs/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'contemplate',
    'look thoughtfully for a long time at',
    '/ˈkɑːntəmpleɪt/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'contemptuous',
    'showing disdain or scorn',
    '/kənˈtɛmptʃuəs/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'contentious',
    'causing or likely to cause disagreement',
    '/kənˈtɛnʃəs/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'controversy',
    'prolonged public disagreement or heated discussion',
    '/ˈkɑːntrəvɜːrsi/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'conventional',
    'based on accepted standards',
    '/kənˈvɛnʃənəl/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'copious',
    'abundant in supply or quantity',
    '/ˈkoʊpiəs/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  ('cordial', 'warm and friendly', '/ˈkɔːrdʒəl/');

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'corroborate',
    'to confirm or give support to',
    '/kəˈrɑːbəˌreɪt/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'credible',
    'able to be believed; convincing',
    '/ˈkredəbl/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'credulity',
    'a tendency to be too ready to believe',
    '/krɪˈduːləti/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'cryptic',
    'having a meaning that is mysterious',
    '/ˈkrɪptɪk/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'culminate',
    'reach a climax or point of highest development',
    '/ˈkʌlməneɪt/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'culpable',
    'deserving blame or censure',
    '/ˈkʌlpəbəl/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'cursory',
    'hasty and therefore not thorough or detailed',
    '/ˈkɜːrsəri/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'cynical',
    'believing that people act purely from self-interest',
    '/ˈsɪnɪkəl/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'debacle',
    'a sudden and ignominious failure',
    '/dɪˈbɑːkəl/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'decorous',
    'in keeping with good taste and propriety',
    '/ˈdɛkərəs/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'decorum',
    'behavior in keeping with good taste and propriety',
    '/dɪˈkɔːrəm/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'defer',
    'to put off to a later time; postpone',
    '/dɪˈfɜr/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'deferential',
    'showing humble submission and respect',
    '/ˌdɛfəˈrɛnʃəl/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'defunct',
    'no longer existing or functioning',
    '/dɪˈfʌŋkt/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'deleterious',
    'causing harm or damage',
    '/ˌdɛlɪˈtɪriəs/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'deliberate',
    'done consciously and intentionally',
    '/dɪˈlɪbərət/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'delineate',
    'to describe or portray precisely',
    '/dɪˈlɪniˌeɪt/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'demeanor',
    'outward behavior or bearing',
    '/dɪˈmiːnər/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'demure',
    'reserved, modest, and shy',
    '/dɪˈmjʊr/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'denigrate',
    'to criticize unfairly; disparage',
    '/ˈdɛnɪˌɡreɪt/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'deplorable',
    'deserving strong condemnation',
    '/dɪˈplɔːrəbəl/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'deplore',
    'feel or express strong disapproval of',
    '/dɪˈplɔːr/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'derogatory',
    'showing a critical or disrespectful attitude',
    '/dɪˈrɑːɡəˌtɔːri/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'desolate',
    'deserted of people and in a state of bleak emptiness',
    '/ˈdesələt/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'despondent',
    'in low spirits from loss of hope',
    '/dɪˈspɑːndənt/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'destitute',
    'without the basic necessities of life',
    '/ˈdɛstɪˌtuːt/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'detrimental',
    'tending to cause harm',
    '/ˌdetrɪˈmentl/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'deviate',
    'depart from an established course',
    '/ˈdiːvieɪt/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'didactic',
    'intended to teach or instruct',
    '/daɪˈdæktɪk/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'diffident',
    'modest or shy because of lack of confidence',
    '/ˈdɪfɪdənt/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'dilatory',
    'slow to act or intended to cause delay',
    '/ˈdɪləˌtɔːri/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'diligent',
    'having or showing care in one s work',
    '/ˈdɪlɪdʒənt/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'diminutive',
    'extremely or unusually small',
    '/dɪˈmɪnjətɪv/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'discern',
    'perceive or recognize something',
    '/dɪˈsɜːrn/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'discerning',
    'having or showing good judgment',
    '/dɪˈsɜːrnɪŋ/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'discordant',
    'disagreeing or incongruous',
    '/dɪsˈkɔːrdənt/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'discrepancy',
    'a lack of compatibility or similarity between two facts',
    '/dɪˈskrepənsi/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'disdain',
    'the feeling that something is unworthy',
    '/dɪsˈdeɪn/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'disparage',
    'to regard or represent as being of little worth',
    '/dɪˈspærɪdʒ/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'disseminate',
    'to spread widely',
    '/dɪˈsɛməˌneɪt/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'divergent',
    'tending to be different or develop in different directions',
    '/daɪˈvɜːrdʒənt/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'doctrine',
    'a belief or set of beliefs held and taught',
    '/ˈdɑːktrɪn/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'dogmatic',
    'inclined to lay down principles as undeniably true',
    '/dɔːɡˈmætɪk/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  ('dormant', 'temporarily inactive', '/ˈdɔːrmənt/');

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  ('dubious', 'hesitating or doubting', '/ˈduːbiəs/');

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'earnest',
    'resulting from or showing sincere and intense conviction',
    '/ˈɜːrnɪst/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'eccentric',
    'unconventional and slightly strange',
    '/ɪkˈsɛntrɪk/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'efficacious',
    'successful in producing a desired result',
    '/ˌɛfɪˈkeɪʃəs/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'elaborate',
    'involving many carefully arranged parts or details',
    '/ɪˈlæbərət/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'eloquent',
    'fluent or persuasive in speaking or writing',
    '/ˈɛləkwənt/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'elucidate',
    'to make something clear; explain',
    '/ɪˈluːsɪdeɪt/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'emaciated',
    'abnormally thin or weak',
    '/ɪˈmeɪʃiˌeɪtɪd/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'embellish',
    'to make something more attractive by decoration',
    '/ɪmˈbɛlɪʃ/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'empirical',
    'based on observation or experience',
    '/ɪmˈpɪrɪkəl/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'emulate',
    'to match or surpass by imitation',
    '/ˈɛmjəˌleɪt/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  ('endorse', 'declare support for', '/ɪnˈdɔːrs/');

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'enervate',
    'to cause someone to feel drained of energy',
    '/ˈɛnərˌveɪt/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'enigma',
    'a person or thing that is mysterious or difficult to understand',
    '/ɪˈnɪɡmə/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'enigmatic',
    'difficult to interpret or understand',
    '/ˌɛnɪɡˈmætɪk/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'entail',
    'involve something as a necessary or inevitable part',
    '/ɪnˈteɪl/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'ephemeral',
    'lasting for a very short time',
    '/ɪˈfɛmərəl/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'epitome',
    'a person or thing that is a perfect example of a quality',
    '/ɪˈpɪtəmi/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'equanimity',
    'mental calmness and composure',
    '/ˌiːkwəˈnɪməti/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  ('equitable', 'fair and impartial', '/ˈekwɪtəbl/');

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'equivocal',
    'open to more than one interpretation',
    '/ɪˈkwɪvəkəl/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  ('erratic', 'unpredictable', '/ɪˈrætɪk/');

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'erudite',
    'having or showing great knowledge',
    '/ˈɛrjəˌdaɪt/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  ('escalate', 'increase rapidly', '/ˈeskəleɪt/');

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'esoteric',
    'intended for or understood by a small group',
    '/ˌɛsəˈtɛrɪk/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'esteemed',
    'held in great respect; admired',
    '/ɪˈstiːmd/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'ethereal',
    'extremely delicate and light',
    '/ɪˈθɪriəl/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'euphemism',
    'a mild expression substituted for a harsh one',
    '/ˈjuːfəˌmɪzəm/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'evanescent',
    'soon passing out of sight or existence',
    '/ˌɛvəˈnɛsənt/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'exacerbate',
    'to make a problem or bad situation worse',
    '/ɪɡˈzæsərˌbeɪt/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'exacting',
    'making great demands on one s skill or attention',
    '/ɪɡˈzæktɪŋ/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'exemplary',
    'serving as a desirable model',
    '/ɪɡˈzempləri/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'exonerate',
    'to absolve someone from blame',
    '/ɪɡˈzɑːnəˌreɪt/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'expedient',
    'convenient and practical despite being improper',
    '/ɪkˈspiːdiənt/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'expedite',
    'to make an action happen sooner',
    '/ˈɛkspɪˌdaɪt/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'explicit',
    'stated clearly and in detail',
    '/ɪkˈsplɪsɪt/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'extol',
    'to praise enthusiastically',
    '/ɪkˈstoʊl/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'extraneous',
    'irrelevant or unrelated to the subject',
    '/ɪkˈstreɪniəs/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'exuberant',
    'filled with or characterized by lively energy',
    '/ɪɡˈzuːbərənt/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'fabricate',
    'invent or concoct something in order to deceive',
    '/ˈfæbrɪkeɪt/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'facilitate',
    'to make an action or process easier',
    '/fəˈsɪlɪteɪt/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'fastidious',
    'very attentive to accuracy and detail',
    '/fæˈstɪdiəs/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  ('fatuous', 'silly and pointless', '/ˈfætʃuəs/');

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'feasible',
    'possible to do easily or conveniently',
    '/ˈfiːzəbl/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'feckless',
    'lacking initiative or strength of character',
    '/ˈfɛkləs/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'fervent',
    'having or displaying passionate intensity',
    '/ˈfɜːrvənt/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'fickle',
    'changing frequently in loyalties or affections',
    '/ˈfɪkəl/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'figurative',
    'departing from literal use of words',
    '/ˈfɪɡjərətɪv/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'flagrant',
    'conspicuously or obviously offensive',
    '/ˈfleɪɡrənt/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'flippant',
    'not showing proper respect or seriousness',
    '/ˈflɪpənt/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'formidable',
    'inspiring fear through being impressively powerful',
    '/ˈfɔːrmɪdəbəl/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'fortuitous',
    'happening by accident or chance',
    '/fɔːrˈtuːɪtəs/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'fractious',
    'irritable and quarrelsome',
    '/ˈfrækʃəs/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'frivolous',
    'not having any serious purpose or value',
    '/ˈfrɪvələs/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'frugal',
    'sparing or economical with regard to money',
    '/ˈfruːɡəl/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'fundamental',
    'forming a necessary base or core',
    '/ˌfʌndəˈmentl/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'futile',
    'incapable of producing any useful result',
    '/ˈfjuːtaɪl/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'garrulous',
    'excessively talkative, especially on trivial matters',
    '/ˈɡɛrələs/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'grandiose',
    'impressive but lacking substance',
    '/ˈɡrændiˌoʊs/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'gratitude',
    'the quality of being thankful',
    '/ˈɡrætɪtuːd/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'gregarious',
    'fond of the company of others',
    '/ɡrɪˈɡɛriəs/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  ('guile', 'sly or cunning intelligence', '/ɡaɪl/');

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'gullible',
    'easily persuaded to believe something',
    '/ˈɡʌləbəl/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'hackneyed',
    'lacking originality or freshness',
    '/ˈhæknid/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'hamper',
    'hinder or impede the movement or progress of',
    '/ˈhæmpər/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'harangue',
    'a lengthy and aggressive speech',
    '/həˈræŋ/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'harmonious',
    'forming a pleasing or consistent whole',
    '/hɑːrˈmoʊniəs/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'haughty',
    'arrogantly superior and disdainful',
    '/ˈhɔːti/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'hedonistic',
    'engaged in the pursuit of pleasure',
    '/ˌhiːdəˈnɪstɪk/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'heretical',
    'believing in or practicing heresy',
    '/həˈrɛtɪkəl/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'heritage',
    'property that is or may be inherited',
    '/ˈherɪtɪdʒ/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'hierarchy',
    'a system in which members are ranked according to status',
    '/ˈhaɪərɑːrki/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'histrionic',
    'overly theatrical or dramatic',
    '/ˌhɪstriˈɑːnɪk/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'hyperbole',
    'exaggerated statements not meant literally',
    '/haɪˈpɜːrbəli/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'hypocritical',
    'behaving in a way that contradicts stated beliefs',
    '/ˌhɪpəˈkrɪtɪkəl/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'hypothetical',
    'based on or serving as a hypothesis',
    '/ˌhaɪpəˈθetɪkl/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'iconoclastic',
    'attacking cherished beliefs or institutions',
    '/aɪˌkɑːnəˈklæstɪk/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'idiosyncratic',
    'relating to idiosyncrasy; peculiar',
    '/ˌɪdiəsɪŋˈkrætɪk/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'ignominious',
    'deserving or causing public disgrace',
    '/ˌɪɡnəˈmɪniəs/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'illicit',
    'forbidden by law, rules, or custom',
    '/ɪˈlɪsɪt/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'illuminate',
    'light up or brighten with light',
    '/ɪˈluːməneɪt/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'immaculate',
    'perfectly clean, neat, or tidy',
    '/ɪˈmækjələt/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'immutable',
    'unchanging over time or unable to be changed',
    '/ɪˈmjuːtəbl/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'impartial',
    'treating all rivals or disputants equally',
    '/ɪmˈpɑːrʃəl/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'impeccable',
    'in accordance with the highest standards',
    '/ɪmˈpɛkəbəl/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'imperative',
    'of vital importance; crucial',
    '/ɪmˈperətɪv/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'imperious',
    'assuming power or authority without justification',
    '/ɪmˈpɪriəs/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'impertinent',
    'not showing proper respect; rude',
    '/ɪmˈpɜːrtɪnənt/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'impetuous',
    'acting quickly without thought or care',
    '/ɪmˈpɛtʃuəs/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'implacable',
    'unable to be placated or appeased',
    '/ɪmˈplækəbəl/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'implicit',
    'suggested though not directly expressed',
    '/ɪmˈplɪsɪt/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'impromptu',
    'done without being planned or rehearsed',
    '/ɪmˈprɑːmptu/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'inadvertent',
    'done without conscious intention',
    '/ˌɪnædˈvɜːrtənt/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'incentive',
    'a thing that motivates or encourages someone to do something',
    '/ɪnˈsentɪv/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'incisive',
    'accurate and sharply focused',
    '/ɪnˈsaɪsɪv/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'incontrovertible',
    'not able to be denied or disputed',
    '/ˌɪnkɑːntrəˈvɜːrtəbəl/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'incumbent',
    'necessary for someone as a duty or responsibility',
    '/ɪnˈkʌmbənt/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'indignant',
    'feeling or showing anger at unfair treatment',
    '/ɪnˈdɪɡnənt/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'indispensable',
    'absolutely necessary',
    '/ˌɪndɪˈspensəbl/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'indolent',
    'wanting to avoid activity or exertion',
    '/ˈɪndələnt/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'ineffable',
    'too great to be expressed in words',
    '/ɪnˈɛfəbəl/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  ('inept', 'showing no skill; clumsy', '/ɪˈnɛpt/');

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'inevitable',
    'certain to happen; unavoidable',
    '/ɪnˈevɪtəbl/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'inexorable',
    'impossible to stop or prevent',
    '/ɪnˈɛksərəbəl/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'infallible',
    'incapable of making mistakes',
    '/ɪnˈfæləbəl/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'infrastructure',
    'the basic physical and organizational structures',
    '/ˈɪnfrəstrʌktʃər/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'ingenuous',
    'innocent and unsuspecting',
    '/ɪnˈdʒɛnjuəs/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'inherent',
    'existing as a natural part of something',
    '/ɪnˈhɪrənt/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'initiate',
    'cause a process or action to begin',
    '/ɪˈnɪʃieɪt/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'innocuous',
    'not harmful or offensive',
    '/ɪˈnɑːkjuəs/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'innovation',
    'the action or process of innovating',
    '/ˌɪnəˈveɪʃn/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'insatiable',
    'impossible to satisfy',
    '/ɪnˈseɪʃəbəl/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'insidious',
    'proceeding in a gradual, subtle way',
    '/ɪnˈsɪdiəs/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'insolent',
    'showing a rude and arrogant lack of respect',
    '/ˈɪnsələnt/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'insuperable',
    'impossible to overcome',
    '/ɪnˈsuːpərəbəl/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'integrity',
    'the quality of being honest and having strong moral principles',
    '/ɪnˈteɡrəti/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'intermittent',
    'occurring at irregular intervals',
    '/ˌɪntərˈmɪtənt/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'intractable',
    'hard to control or deal with',
    '/ɪnˈtræktəbəl/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'intrepid',
    'fearless; adventurous',
    '/ɪnˈtrɛpɪd/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'intricate',
    'very complicated or detailed',
    '/ˈɪntrɪkət/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'intrinsic',
    'belonging naturally; essential',
    '/ɪnˈtrɪnzɪk/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'intuitive',
    'using or based on what one feels to be true',
    '/ɪnˈtuːɪtɪv/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'inundate',
    'to overwhelm with things to be dealt with',
    '/ˈɪnəndeɪt/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'invincible',
    'too powerful to be defeated',
    '/ɪnˈvɪnsəbəl/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'irascible',
    'having a tendency to be easily angered',
    '/ɪˈræsəbəl/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'irrelevant',
    'not connected with or relevant to something',
    '/ɪˈrɛləvənt/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'irreverent',
    'showing a lack of respect for things',
    '/ɪˈrɛvərənt/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'itinerant',
    'traveling from place to place',
    '/aɪˈtɪnərənt/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'jaded',
    'tired, bored, or lacking enthusiasm',
    '/ˈdʒeɪdɪd/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'jeopardize',
    'put someone or something into a situation of loss or danger',
    '/ˈdʒepərdaɪz/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'jocular',
    'fond of or characterized by joking',
    '/ˈdʒɑːkjələr/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'judicious',
    'having, showing, or done with good judgment',
    '/dʒuˈdɪʃəs/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'juxtapose',
    'to place or deal with close together for contrasting effect',
    '/ˌdʒʌkstəˈpoʊz/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'kaleidoscope',
    'a constantly changing pattern or sequence of elements',
    '/kəˈlaɪdəˌskoʊp/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'laconic',
    'using few words; concise',
    '/ləˈkɑːnɪk/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'languid',
    'displaying or having a disinclination for physical exertion or effort',
    '/ˈlæŋɡwɪd/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'latent',
    'existing but not yet developed or manifest',
    '/ˈleɪtənt/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'laudable',
    'deserving praise and commendation',
    '/ˈlɔːdəbəl/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'legacy',
    'an amount of money or property left to someone in a will',
    '/ˈleɡəsi/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'legitimate',
    'conforming to the law or to rules',
    '/lɪˈdʒɪtəmət/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'lethargic',
    'affected by lethargy; sluggish',
    '/ləˈθɑːrdʒɪk/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'liability',
    'the state of being responsible for something',
    '/ˌlaɪəˈbɪləti/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'loquacious',
    'tending to talk a great deal',
    '/loʊˈkweɪʃəs/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'lucid',
    'expressed clearly; easy to understand',
    '/ˈluːsɪd/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'lucrative',
    'producing a great deal of profit',
    '/ˈluːkrətɪv/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'lugubrious',
    'looking or sounding sad and dismal',
    '/ləˈɡuːbriəs/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'magnanimous',
    'generous or forgiving toward a rival',
    '/mæɡˈnænəməs/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'malevolent',
    'having or showing a wish to do evil',
    '/məˈlɛvələnt/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'malleable',
    'able to be shaped; easily influenced',
    '/ˈmæliəbəl/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'manifest',
    'clear or obvious to the eye or mind',
    '/ˈmænəˌfɛst/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'maverick',
    'an unorthodox or independent-minded person',
    '/ˈmævərɪk/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'meander',
    'follow a winding course',
    '/miˈændər/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'mediocre',
    'of only moderate quality; not very good',
    '/ˌmiːdiˈoʊkər/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'mendacious',
    'given to or characterized by deception',
    '/mɛnˈdeɪʃəs/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'mercurial',
    'subject to sudden changes of mood',
    '/mərˈkjʊriəl/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'methodical',
    'done according to a systematic plan',
    '/məˈθɑːdɪkəl/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'meticulous',
    'showing great attention to detail; very careful and precise',
    '/məˈtɪkjələs/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'mitigate',
    'to make less severe, serious, or painful',
    '/ˈmɪtɪˌɡeɪt/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'modicum',
    'a small quantity of a particular thing',
    '/ˈmɑːdɪkəm/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'mollify',
    'to make someone less angry or upset',
    '/ˈmɑːləˌfaɪ/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  ('morose', 'bad-tempered and sulky', '/məˈroʊs/');

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'mundane',
    'lacking interest or excitement; dull',
    '/mʌnˈdeɪn/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'myriad',
    'a countless or extremely great number',
    '/ˈmɪriəd/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'nascent',
    'just coming into existence',
    '/ˈnæsənt/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'nebulous',
    'in the form of a cloud; hazy',
    '/ˈnɛbjələs/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  ('nefarious', 'wicked or criminal', '/nəˈfɛriəs/');

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'negligible',
    'so small as to be not worth considering',
    '/ˈnɛɡlɪdʒəbəl/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'nomadic',
    'living the life of a nomad; wandering',
    '/noʊˈmædɪk/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'nonchalant',
    'feeling or appearing casually calm',
    '/ˌnɑːnʃəˈlɑːnt/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'nostalgic',
    'feeling sentimental longing for the past',
    '/nəˈstældʒɪk/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'notorious',
    'famous or well known for a bad quality',
    '/noʊˈtɔːriəs/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'nuance',
    'a subtle difference in expression or meaning',
    '/ˈnuːɑːns/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'obfuscate',
    'to deliberately make something difficult to understand',
    '/ˈɒbfəˌskeɪt/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'objective',
    'not influenced by personal feelings or opinions',
    '/əbˈdʒektɪv/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'obsequious',
    'obedient or attentive to an excessive degree',
    '/əbˈsiːkwiəs/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'obsolete',
    'no longer produced or used; out of date',
    '/ˌɑːbsəˈliːt/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'obstinate',
    'stubbornly refusing to change opinion',
    '/ˈɑːbstɪnət/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'obtuse',
    'annoyingly insensitive or slow to understand',
    '/əbˈtuːs/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'ominous',
    'giving the impression that something bad will happen',
    '/ˈɑːmɪnəs/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'onerous',
    'involving an amount of effort that is oppressive',
    '/ˈoʊnərəs/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'optimistic',
    'hopeful and confident about the future',
    '/ˌɑːptɪˈmɪstɪk/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'opulent',
    'ostentatiously rich and luxurious',
    '/ˈɑːpjələnt/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'ornate',
    'made in an intricate shape or decorated',
    '/ɔːrˈneɪt/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'orthodox',
    'following traditional beliefs or practices',
    '/ˈɔːrθəˌdɑːks/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'ostentatious',
    'characterized by vulgar display',
    '/ˌɑːstənˈteɪʃəs/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'palpable',
    'able to be touched or felt',
    '/ˈpælpəbəl/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'paradigm',
    'a typical example or pattern of something; a model',
    '/ˈpærəˌdaɪm/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'paragon',
    'a person or thing regarded as perfect',
    '/ˈpærəˌɡɑːn/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'paramount',
    'of the highest importance',
    '/ˈpærəˌmaʊnt/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'parsimonious',
    'extremely frugal; unwilling to spend',
    '/ˌpɑːrsɪˈmoʊniəs/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'pastoral',
    'relating to the countryside; rural',
    '/ˈpæstərəl/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'pathos',
    'a quality that evokes pity or sadness',
    '/ˈpeɪθɑːs/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'patronizing',
    'apparently kind but betraying condescension',
    '/ˈpeɪtrəˌnaɪzɪŋ/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'paucity',
    'the presence of something only in small quantities',
    '/ˈpɔːsəti/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'pedantic',
    'excessively concerned with minor details',
    '/pəˈdæntɪk/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'pejorative',
    'expressing contempt or disapproval',
    '/pəˈdʒɔːrətɪv/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'penchant',
    'a strong or habitual liking for something',
    '/ˈpɛnʃənt/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'pensive',
    'engaged in deep or serious thought',
    '/ˈpɛnsɪv/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'perennial',
    'lasting or existing for a long time',
    '/pəˈrɛniəl/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'perfunctory',
    'carried out with minimum effort',
    '/pərˈfʌŋktəri/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'pernicious',
    'having a harmful effect in a gradual way',
    '/pərˈnɪʃəs/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'perspicacious',
    'having keen insight; discerning',
    '/ˌpɜːrspɪˈkeɪʃəs/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'pertinent',
    'relevant or applicable to a particular matter',
    '/ˈpɜːrtɪnənt/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'pervasive',
    'spreading widely throughout an area',
    '/pərˈveɪsɪv/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'petulant',
    'childishly sulky or bad-tempered',
    '/ˈpɛtʃələnt/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'philanthropic',
    'seeking to promote welfare of others',
    '/ˌfɪlənˈθrɑːpɪk/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'placid',
    'not easily upset or excited',
    '/ˈplæsɪd/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'plausible',
    'seeming reasonable or probable',
    '/ˈplɔːzəbəl/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'poignant',
    'evoking a keen sense of sadness',
    '/ˈpɔɪnjənt/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'pompous',
    'affectedly grand or self-important',
    '/ˈpɑːmpəs/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'ponderous',
    'having great weight; heavy and slow',
    '/ˈpɑːndərəs/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'pragmatic',
    'dealing with things sensibly and realistically',
    '/præɡˈmætɪk/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'precedent',
    'an earlier event or action that is regarded as an example',
    '/ˈpresɪdənt/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'precipitous',
    'done with great haste',
    '/prɪˈsɪpɪtəs/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'precocious',
    'having developed abilities at an early age',
    '/prɪˈkoʊʃəs/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'predilection',
    'a preference or special liking for something',
    '/ˌpriːdəˈlɛkʃən/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'presumptuous',
    'failing to observe proper respect',
    '/prɪˈzʌmptʃuəs/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'pretentious',
    'attempting to impress by affecting importance',
    '/prɪˈtɛnʃəs/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'prevalent',
    'widespread in a particular area at a particular time',
    '/ˈprevələnt/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'pristine',
    'in its original condition; unspoiled',
    '/ˈprɪstiːn/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'proficient',
    'competent or skilled in doing or using something',
    '/prəˈfɪʃənt/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'profound',
    'very great or intense',
    '/prəˈfaʊnd/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'prolific',
    'present in large numbers or quantities',
    '/prəˈlɪfɪk/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  ('prominent', 'important; famous', '/ˈprɑːmənənt/');

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'propensity',
    'an inclination or natural tendency',
    '/prəˈpɛnsəti/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'prosaic',
    'lacking poetic beauty; commonplace',
    '/proʊˈzeɪɪk/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'provincial',
    'of or concerning a province of a country',
    '/prəˈvɪnʃəl/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'prudent',
    'acting with care and thought for the future',
    '/ˈpruːdənt/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'puerile',
    'childishly silly and trivial',
    '/ˈpjʊrəl/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'punctilious',
    'showing great attention to detail',
    '/pʌŋkˈtɪliəs/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'quaint',
    'attractively unusual or old-fashioned',
    '/kweɪnt/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'querulous',
    'complaining in a petulant way',
    '/ˈkwɛrələs/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'quiescent',
    'in a state of inactivity or dormancy',
    '/kwiˈɛsənt/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'quintessential',
    'representing the most perfect or typical example of a quality or class',
    '/ˌkwɪntɪˈsɛnʃəl/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'rancorous',
    'characterized by bitterness or resentment',
    '/ˈræŋkərəs/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'rapacious',
    'aggressively greedy or grasping',
    '/rəˈpeɪʃəs/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'rapport',
    'a close and harmonious relationship',
    '/ræˈpɔːr/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'recalcitrant',
    'having an obstinately uncooperative attitude',
    '/rɪˈkælsɪtrənt/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'reciprocal',
    'given, felt, or done in return',
    '/rɪˈsɪprəkl/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'reclusive',
    'avoiding the company of other people',
    '/rɪˈkluːsɪv/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'redundant',
    'not needed because of repetition',
    '/rɪˈdʌndənt/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'refute',
    'to prove a statement wrong',
    '/rɪˈfjuːt/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'relegate',
    'to consign to an inferior position',
    '/ˈrɛləˌɡeɪt/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'relevant',
    'closely connected or appropriate to what is being done',
    '/ˈreləvənt/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'remiss',
    'lacking care or attention to duty',
    '/rɪˈmɪs/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'reprehensible',
    'deserving censure or condemnation',
    '/ˌrɛprɪˈhɛnsəbəl/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'repudiate',
    'to refuse to accept or be associated with',
    '/rɪˈpjuːdiˌeɪt/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'resilient',
    'able to withstand or recover quickly from difficult conditions',
    '/rɪˈzɪljənt/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'resolute',
    'admirably purposeful and determined',
    '/ˈrɛzəˌluːt/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'reticent',
    'not revealing thoughts or feelings readily',
    '/ˈrɛtəsənt/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'reverent',
    'feeling or showing deep respect',
    '/ˈrɛvərənt/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'rhetoric',
    'the art of effective or persuasive speaking',
    '/ˈrɛtərɪk/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'rigorous',
    'extremely thorough and careful',
    '/ˈrɪɡərəs/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'rudimentary',
    'involving basic principles',
    '/ˌruːdəˈmɛntəri/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'sagacious',
    'having or showing keen mental discernment and good judgment',
    '/səˈɡeɪʃəs/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'sagacity',
    'the quality of being sage; wisdom',
    '/səˈɡæsəti/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'salutary',
    'producing good effects; beneficial',
    '/ˈsæljəˌtɛri/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'sanctimonious',
    'making a show of being morally superior',
    '/ˌsæŋktəˈmoʊniəs/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'sanguine',
    'optimistic or positive in a difficult situation',
    '/ˈsæŋɡwɪn/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'sardonic',
    'grimly mocking or cynical',
    '/sɑːrˈdɑːnɪk/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'scrupulous',
    'diligent, thorough, and extremely attentive',
    '/ˈskruːpjələs/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'scrutinize',
    'examine or inspect closely and thoroughly',
    '/ˈskruːtənaɪz/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'sedentary',
    'tending to spend much time seated',
    '/ˈsɛdnˌtɛri/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'sedulous',
    'showing dedication and diligence',
    '/ˈsɛdʒələs/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'seminal',
    'strongly influencing later developments',
    '/ˈsɛmənəl/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'serendipity',
    'occurrence of events by chance in a happy way',
    '/ˌsɛrənˈdɪpəti/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'servile',
    'having or showing excessive willingness to serve',
    '/ˈsɜːrvaɪl/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  ('shoddy', 'badly made or done', '/ˈʃɑːdi/');

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'simulate',
    'imitate the appearance or character of',
    '/ˈsɪmjəleɪt/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'skeptical',
    'not easily convinced; having doubts',
    '/ˈskɛptɪkəl/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  ('solemn', 'formal and dignified', '/ˈsɑːləm/');

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'solicitous',
    'characterized by care and concern',
    '/səˈlɪsətəs/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'somber',
    'dark or dull in color; grave',
    '/ˈsɑːmbər/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'soporific',
    'tending to induce drowsiness or sleep',
    '/ˌsɑːpəˈrɪfɪk/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'sordid',
    'involving immoral or dishonest behavior',
    '/ˈsɔːrdɪd/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'spontaneous',
    'performed without premeditation',
    '/spɑːnˈteɪniəs/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'spurious',
    'not being what it purports to be; false',
    '/ˈspjʊriəs/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'stagnant',
    'having no current or flow; not developing',
    '/ˈstæɡnənt/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'stoic',
    'a person who endures hardship without complaint',
    '/ˈstoʊɪk/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'stringent',
    'demanding exact conformity to rules',
    '/ˈstrɪndʒənt/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'stupefy',
    'to make someone unable to think clearly',
    '/ˈstuːpəˌfaɪ/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'sublime',
    'of such excellence as to inspire awe',
    '/səˈblaɪm/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'subsequent',
    'coming after something in time',
    '/ˈsʌbsəkwənt/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'substantial',
    'of considerable importance, size, or worth',
    '/səbˈstænʃl/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'substantiate',
    'to provide evidence to support or prove',
    '/səbˈstænʃiˌeɪt/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'subtle',
    'so delicate as to be difficult to analyze',
    '/ˈsʌtəl/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'succinct',
    'briefly and clearly expressed',
    '/səkˈsɪŋkt/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'superficial',
    'existing or occurring at or on the surface',
    '/ˌsuːpərˈfɪʃl/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'superfluous',
    'unnecessary, especially through being more than enough',
    '/suːˈpɜːrfluəs/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'supplant',
    'to supersede and replace',
    '/səˈplænt/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'surreptitious',
    'kept secret, especially because it would not be approved',
    '/ˌsɜːrəpˈtɪʃəs/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'sycophant',
    'a person who acts obsequiously to gain advantage',
    '/ˈsɪkəfənt/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'taciturn',
    'reserved or uncommunicative in speech; saying little',
    '/ˈtæsɪˌtɜrn/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'tangible',
    'perceptible by touch; clear and definite',
    '/ˈtændʒəbəl/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'tedious',
    'too long, slow, or dull; tiresome',
    '/ˈtiːdiəs/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'temerity',
    'excessive confidence or boldness',
    '/təˈmɛrəti/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'tenacious',
    'tending to keep a firm hold of something',
    '/təˈneɪʃəs/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'tentative',
    'not certain or fixed; provisional',
    '/ˈtɛntətɪv/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  ('tepid', 'showing little enthusiasm', '/ˈtɛpɪd/');

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'terse',
    'sparing in the use of words; abrupt',
    '/tɜːrs/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'torpid',
    'mentally or physically inactive',
    '/ˈtɔːrpɪd/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'tractable',
    'easy to control or influence',
    '/ˈtræktəbəl/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'tranquil',
    'free from disturbance; calm',
    '/ˈtræŋkwəl/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'transient',
    'lasting only for a short time',
    '/ˈtrænʃənt/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'trepidation',
    'a feeling of fear about something that may happen',
    '/ˌtrɛpəˈdeɪʃən/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'trivial',
    'of little worth or importance',
    '/ˈtrɪviəl/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'truculent',
    'eager or quick to argue or fight',
    '/ˈtrʌkjələnt/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'turbulent',
    'very disturbed or characterized by conflict',
    '/ˈtɜːrbjələnt/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'turgid',
    'swollen and distended or congested',
    '/ˈtɜːrdʒɪd/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'ubiquitous',
    'present, appearing, or found everywhere',
    '/juˈbɪkwɪtəs/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'ubiquity',
    'the fact of appearing everywhere',
    '/juːˈbɪkwəti/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'uncouth',
    'lacking good manners or refinement',
    '/ʌnˈkuːθ/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'unctuous',
    'excessively flattering or ingratiating',
    '/ˈʌŋktʃuəs/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'undermine',
    'to erode the base or foundation of something',
    '/ˌʌndərˈmaɪn/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'unequivocal',
    'leaving no doubt; unambiguous',
    '/ˌʌnɪˈkwɪvəkəl/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'unfettered',
    'unrestrained or uninhibited',
    '/ʌnˈfɛtərd/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'unilateral',
    'performed by or affecting only one person or side',
    '/ˌjuːnəˈlætərəl/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'unprecedented',
    'never done or known before',
    '/ʌnˈprɛsəˌdɛntɪd/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'unscrupulous',
    'having or showing no moral principles',
    '/ʌnˈskruːpjələs/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'unwieldy',
    'difficult to carry or move because of its size',
    '/ʌnˈwiлdi/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'urbane',
    'suave, courteous, and refined in manner',
    '/ɜːrˈbeɪn/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'usurp',
    'to take someone s position illegally',
    '/juːˈzɜːrp/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'vacillate',
    'to alternate between different opinions',
    '/ˈvæsəˌleɪt/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'vacuous',
    'having or showing a lack of thought',
    '/ˈvækjuəs/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'vapid',
    'offering nothing that is stimulating',
    '/ˈvæpɪd/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'vehement',
    'showing strong feeling; forceful',
    '/ˈviəmənt/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'venerable',
    'accorded respect because of age or wisdom',
    '/ˈvɛnərəbəl/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'verbose',
    'using more words than needed',
    '/vərˈboʊs/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'versatile',
    'able to adapt or be adapted to many different functions',
    '/ˈvɜːrsətl/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'viable',
    'capable of working successfully',
    '/ˈvaɪəbəl/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'vicarious',
    'experienced in the imagination through the feelings or actions of another person',
    '/vɪˈkɛəriəs/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'vindicate',
    'to clear someone of blame or suspicion',
    '/ˈvɪndəˌkeɪt/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'virulent',
    'extremely severe or harmful in its effects',
    '/ˈvɪrjələnt/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'visceral',
    'relating to deep inward feelings',
    '/ˈvɪsərəl/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'vitriolic',
    'filled with bitter criticism or malice',
    '/ˌvɪtriˈɑːlɪk/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'vivacious',
    'attractively lively and animated',
    '/vɪˈveɪʃəs/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'vociferous',
    'expressing opinions in a loud way',
    '/voʊˈsɪfərəs/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'voluble',
    'speaking fluently and readily',
    '/ˈvɑːljəbəl/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'voracious',
    'wanting or devouring great quantities',
    '/vəˈreɪʃəs/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'wary',
    'feeling or showing caution about possible dangers or problems',
    '/ˈwɛəri/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'whimsical',
    'playfully quaint or fanciful',
    '/ˈwɪmzɪkəl/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'wistful',
    'having a feeling of vague longing',
    '/ˈwɪstfəl/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'zealous',
    'having or showing zeal; passionate',
    '/ˈzɛləs/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'zenith',
    'the time at which something is most powerful',
    '/ˈziːnɪθ/'
  );

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  ('zephyr', 'a gentle mild breeze', '/ˈzɛfər/');

INSERT INTO
  words (word, definition, pronunciation)
VALUES
  (
    'zodiac',
    'a belt of the heavens within which lie the paths of the sun, moon, and planets',
    '/ˈzoʊdiæk/'
  );
