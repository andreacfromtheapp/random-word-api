...
#[tokio::test]
async fn test_random_word_endpoint_optimized() -> Result<()> {
...
#[tokio::test]
async fn test_all_word_types_batch_optimized() -> Result<()> {
    let server = create_test_server_streamlined().await?;

    // Batch test all word types for efficiency
...
#[tokio::test]
async fn test_api_response_format_compliance() -> Result<()> {
    let server = create_test_server_streamlined().await?;
...
#[tokio::test]
async fn test_invalid_requests_batch() -> Result<()> {
    let server = create_test_server_streamlined().await?;
...
#[tokio::test]
async fn test_error_handling_functionality() -> Result<()> {
    let server = create_test_server_streamlined().await?;
...
async fn test_multiple_requests_reliability() -> Result<()> {
    ...
    // Test multiple sequential requests for reliability (minimal iterations)
    for _ in 0..3 {
        let response = server.get("/en/word").await;
        assert!(
            response.status_code() == StatusCode::OK
...
async fn test_api_consistency() -> Result<()> {
    ...
    for _ in 0..3 {
        ...
        let json: serde_json::Value = response.json();
        assert!(json.is_array(), "All responses should be arrays");
    }
...
#[tokio::test]
async fn test_optimized_api_batch() -> Result<()> {
    let server = create_test_server_streamlined().await?;
...
#[tokio::test]
async fn test_workflow_scenarios() -> Result<()> {
    let server = create_test_server_streamlined().await?;
